            ...
            toast.innerHTML = `
                <div class="px-6 py-3 rounded-lg shadow-lg transform transition-all duration-300 ${
                    ...
                    <div class="flex items-center">
                        <i data-feather="${type === 'success' ? 'check-circle' : type === 'error' ? 'alert-circle' : type === 'warning' ? 'alert-triangle' : 'info'}" 
        ...
        function renderUsersTable() {
            ...
            document.getElementById('usersTotalItems').textContent = filteredUsersData.length;
        ...
        function updateUsersPagination() {
            ...
            for (let i = startPage; i <= endPage; i++) {
                paginationHTML += `
                    <button class="users-page-btn px-3 py-1 border ${i === currentUsersPage ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300'} rounded text-sm" 
                            data-page="${i}">
                        ${i}
                    </button>
                `;
        ...
        function getRoleInfo(role) {
            const roles = {
                'admin': { text: 'Administrateur', color: 'bg-purple-100 text-purple-800', icon: 'shield' },
                'doctor': { text: 'Médecin', color: 'bg-blue-100 text-blue-800', icon: 'user' },
                'staff': { text: 'Personnel', color: 'bg-green-100 text-green-800', icon: 'users' },
        ...
        function getStatusInfo(status) {
            const statuses = {
                ...
                'suspended': { text: 'Suspendu', color: 'bg-orange-100 text-orange-800', icon: 'pause-circle' }
            };
            return statuses[status] || { text: status, color: 'bg-gray-100 text-gray-800', icon: 'help-circle' };
        }
            ...
            try {
                servicesData = [
                    { 
                        ...
                        name: "Radiologie", 
                        code: "SERV-RAD-001",
                        description: "Service d'imagerie médicale équipé des dernières technologies", 
                        status: "active", 
                        createdAt: "2024-01-15",
                        hospital: "Hôpital Central EMSI",
                        ...
                        equipment: ["Scanner 64 barrettes", "IRM 3 Tesla", "Échographie 4D", "Radiologie numérique"]
                    },
                    { 
                        id: 2, 
                        name: "Cardiologie", 
                        ...
                        equipment: ["Échocardiographie", "Holter ECG", "Moniteur tensionnel", "Défibrillateur"]
                    },
                    { 
                        id: 3, 
                        name: "Urgences", 
                        code: "SERV-URG-003",
                        description: "Service d'urgences médicales 24h/24 avec équipement de réanimation", 
                        status: "active", 
                        createdAt: "2024-03-05",
                        hospital: "Clinique Al Farabi",
                        ...
                        equipment: ["Salles de réanimation", "Matériel d'intubation", "Moniteurs multiparamètres", "Échographe portable"]
                    }
                ];
        ...
        function filterAndPaginateServices() {
            ...
            const searchTerm = document.getElementById('searchServicesTable').value.toLowerCase();
        ...
        function renderServicesTable() {
            ...
            const endIndex = Math.min(startIndex + servicesPerPage, filteredServicesData.length);
                ...
                html += `
                    <tr class="service-row" data-service-id="${service.id}">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-sm text-gray-500">${service.hospital}</div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${service.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                                <i data-feather="${service.status === 'active' ? 'check-circle' : 'x-circle'}" class="w-3 h-3 mr-1"></i>
            ...
            document.getElementById('servicesTotalItems').textContent = filteredServicesData.length;
        ...
        function updateServicesPagination() {
            ...
            const paginationNumbers = document.getElementById('servicesPaginationNumbers');
            ...
            let paginationHTML = '';
            const maxVisiblePages = 5;
            let startPage = Math.max(1, currentServicesPage - Math.floor(maxVisiblePages / 2));
            let endPage = Math.min(totalPages, startPage + maxVisiblePages - 1);
            ...
            for (let i = startPage; i <= endPage; i++) {
                paginationHTML += `
                    <button class="services-page-btn px-3 py-1 border ${i === currentServicesPage ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300'} rounded text-sm" 
                            data-page="${i}">
                        ${i}
                    </button>
                `;
        ...
        function openViewUserModal(userId) {
            ...
            document.getElementById('viewUserBirthDate').textContent = user.birthDate ? formatDate(user.birthDate) : '-';
            ...
            document.getElementById('viewUserHospital').textContent = user.hospital || '-';
        ...
        function openViewServiceModal(serviceId) {
            ...
            document.getElementById('viewServiceCode').textContent = service.code || generateServiceCode(service.id);
            document.getElementById('viewServiceHospital').textContent = service.hospital || '-';
            document.getElementById('viewServiceChief').textContent = service.chief || '-';
            document.getElementById('viewServiceDescription').textContent = service.description || '-';
            document.getElementById('viewServiceCreatedAt').textContent = formatDate(service.createdAt);
            
            const statusText = service.status === 'active' ? 'Actif' : 
                             service.status === 'maintenance' ? 'En maintenance' : 'Fermé';
            ...
            const statusIcon = service.status === 'active' ? 'check-circle' : 
                             service.status === 'maintenance' ? 'tool' : 'x-circle';
            
            document.getElementById('viewServiceStatus').innerHTML = `
                <i data-feather="${statusIcon}" class="w-4 h-4 mr-1"></i>
                ${statusText}
            `;
            document.getElementById('viewServiceStatus').className = `inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${statusColor}`;
            
            document.getElementById('viewServiceTotalBeds').textContent = service.totalBeds || 0;
            document.getElementById('viewServiceOccupiedBeds').textContent = service.occupiedBeds || 0;
            const occupationRate = service.totalBeds ? Math.round((service.occupiedBeds || 0) / service.totalBeds * 100) : 0;
            document.getElementById('viewServiceOccupationRate').textContent = `${occupationRate}%`;
                ...
                service.assignedDoctors.forEach(doctor => {
                    ...
                    doctorsContainer.innerHTML += `
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${roleColors[doctor.role] || 'bg-gray-100 text-gray-800'}">
                                ${doctor.role === 'chief' ? 'Chef de service' : 
                                  doctor.role === 'regular' ? 'Médecin' : 
                                  doctor.role === 'assistant' ? 'Assistant' : 'Spécialiste'}
                            </span>
                        </div>
                    `;
                });
        ...
        function openEditServiceModal(serviceId) {
            ...
            document.getElementById('editServiceHospital').value = service.hospital || '1';
            document.getElementById('editServiceChief').value = service.chief || '1';
            document.getElementById('editServiceTotalBeds').value = service.totalBeds || 10;
            document.getElementById('editServiceStatus').value = service.status || 'active';
            document.getElementById('editServiceDescription').value = service.description || '';
            document.getElementById('editServiceEquipment').value = service.equipment ? service.equipment.join(', ') : '';
        ...
        function openDeleteServiceModal(serviceId) {
            ...
            document.getElementById('deleteServiceInfo').textContent = `${service.hospital || 'Hôpital'} • ${service.totalBeds || 0} lits`;
            document.getElementById('deleteServiceChief').textContent = service.chief || '-';
            const occupationRate = service.totalBeds ? Math.round((service.occupiedBeds || 0) / service.totalBeds * 100) : 0;
            document.getElementById('deleteServiceOccupation').textContent = `${occupationRate}% (${service.occupiedBeds || 0}/${service.totalBeds || 0} lits)`;
            
            const warningText = service.occupiedBeds > 0 ? 
        ...
        function openAssignDoctorModal(serviceId) {
            ...
            document.getElementById('assignServiceInfo').textContent = 
                `${service.hospital || 'Hôpital'} • ${service.code || generateServiceCode(service.id)}`;
            document.getElementById('assignServiceId').value = serviceId;

            const doctors = usersData.filter(user => user.role === 'doctor' && user.status === 'active');
            const select = document.getElementById('assignDoctorSelect');
            
            let options = '<option value="">Choisir un médecin</option>';
            doctors.forEach(doctor => {
                const fullName = `${doctor.firstName} ${doctor.lastName}`;
                const isAlreadyAssigned = service.assignedDoctors?.some(d => d.id === doctor.id);
                
                options += `
                    <option value="${doctor.id}" ${isAlreadyAssigned ? 'disabled' : ''}>
                        ${fullName}${doctor.specialty ? ` (${doctor.specialty})` : ''}
        ...
        function setupUsersEvents() {
            // Filtres
            document.getElementById('filterRole')?.addEventListener('change', filterAndPaginateUsers);
            document.getElementById('filterStatus')?.addEventListener('change', filterAndPaginateUsers);
            document.getElementById('searchUsers')?.addEventListener('input', filterAndPaginateUsers);
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.users-page-btn')) {
                    const page = parseInt(e.target.closest('.users-page-btn').getAttribute('data-page'));
                ...
                if (e.target.closest('.services-page-btn')) {
                    const page = parseInt(e.target.closest('.services-page-btn').getAttribute('data-page'));
            ...
            document.getElementById('editUserForm')?.addEventListener('submit', async (e) => {
                ...
                usersData[userIndex] = {
                    ...
                    hospital: document.getElementById('editUserHospital').value === '1' ? 'Hôpital Central EMSI' : 
                              document.getElementById('editUserHospital').value === '2' ? 'Hôpital Ibn Sina' : 'Clinique Al Farabi'
                };
            ...
            document.getElementById('closeViewUserModal2')?.addEventListener('click', () => {
                closeModal('viewUserModal');
            });
            ...
            document.getElementById('editFromViewUserBtn')?.addEventListener('click', () => {
                closeModal('viewUserModal');
        ...
        function setupServicesEvents() {
            // Filtres
            document.getElementById('filterServiceStatus')?.addEventListener('change', filterAndPaginateServices);
            document.getElementById('searchServicesTable')?.addEventListener('input', filterAndPaginateServices);
            ...
            // Items per page
            document.getElementById('servicesPerPage')?.addEventListener('change', (e) => {
            ...
            document.getElementById('addServiceForm')?.addEventListener('submit', async (e) => {
                ...
                const equipmentText = document.getElementById('addServiceEquipment').value;
                const equipment = equipmentText ? equipmentText.split(',').map(item => item.trim()).filter(item => item) : [];
                ...
                const newService = {
                    ...
                    createdAt: new Date().toISOString().split('T')[0],
                    hospital: document.getElementById('addServiceHospital').value === '1' ? 'Hôpital Central EMSI' : 
                             document.getElementById('addServiceHospital').value === '2' ? 'Hôpital Ibn Sina' : 'Clinique Al Farabi',
                    chief: document.getElementById('addServiceChief').value === '1' ? 'Dr. Ahmed Benani' : 
                          document.getElementById('addServiceChief').value === '2' ? 'Dr. Fatima Zahra' : 'Dr. Karim Alami',
                    totalBeds: parseInt(document.getElementById('addServiceTotalBeds').value),
                    occupiedBeds: 0,
                    assignedDoctors: [],
                    equipment: equipment
                };
            ...
            document.getElementById('editServiceForm')?.addEventListener('submit', async (e) => {
                ...
                const equipmentText = document.getElementById('editServiceEquipment').value;
                const equipment = equipmentText ? equipmentText.split(',').map(item => item.trim()).filter(item => item) : [];
                ...
                servicesData[serviceIndex] = {
                    ...
                    hospital: document.getElementById('editServiceHospital').value === '1' ? 'Hôpital Central EMSI' : 
                             document.getElementById('editServiceHospital').value === '2' ? 'Hôpital Ibn Sina' : 'Clinique Al Farabi',
                    chief: document.getElementById('editServiceChief').value === '1' ? 'Dr. Ahmed Benani' : 
                          document.getElementById('editServiceChief').value === '2' ? 'Dr. Fatima Zahra' : 'Dr. Karim Alami',
                    totalBeds: parseInt(document.getElementById('editServiceTotalBeds').value),
                    equipment: equipment
                };
            ...
            document.getElementById('cancelAddServiceBtn')?.addEventListener('click', () => {
                closeModal('addServiceModal');
            });

            document.getElementById('closeEditServiceModal')?.addEventListener('click', () => {
                closeModal('editServiceModal');
            });
            ...
            document.getElementById('closeViewServiceModal')?.addEventListener('click', () => {
                closeModal('viewServiceModal');
            });

            document.getElementById('closeViewServiceModal2')?.addEventListener('click', () => {
                closeModal('viewServiceModal');
            });

            document.getElementById('cancelDeleteServiceBtn')?.addEventListener('click', () => {
                closeModal('deleteServiceModal');
            });

            document.getElementById('editFromViewServiceBtn')?.addEventListener('click', () => {
                closeModal('viewServiceModal');
                openEditServiceModal(currentEditingServiceId);
            });

            // Gestion de l'affectation des médecins
            document.getElementById('assignDoctorForm')?.addEventListener('submit', handleDoctorAssignment);
            
            document.getElementById('closeAssignDoctorModal')?.addEventListener('click', () => {
                closeModal('assignDoctorModal');
            });
            
            document.getElementById('cancelAssignDoctorBtn')?.addEventListener('click', () => {
                closeModal('assignDoctorModal');
            });
            ...
            document.addEventListener('click', async (e) => {
                if (e.target.closest('.assign-doctor-btn')) {
                    const serviceId = e.target.closest('.assign-doctor-btn').getAttribute('data-service-id');
                    openAssignDoctorModal(serviceId);
                }
                
                if (e.target.closest('.edit-service-btn')) {
                    const serviceId = e.target.closest('.edit-service-btn').getAttribute('data-service-id');
                    openEditServiceModal(serviceId);
                }
                
                if (e.target.closest('.delete-service-btn')) {
                    const serviceId = e.target.closest('.delete-service-btn').getAttribute('data-service-id');
                    openDeleteServiceModal(serviceId);
                }
                
                if (e.target.closest('.view-service-btn')) {
                    const serviceId = e.target.closest('.view-service-btn').getAttribute('data-service-id');
